...
if 'mnist' in args.data:
    ...
    testdir = os.path.join(args.dir, 'mnist')
...
for task_id in range(num_task+1):
    ...
    if 'imagenet' in args.data:
        ...
        test_loader = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=0,drop_last=False)
    elif args.data == 'cifar10':
        np.random.seed(args.seed)
        target_transform = np.random.permutation(num_classes)
        testset = CIFAR10(root=testdir, train=False, download=True, transform=transform_test,
                           target_transform=None, index=index)
        test_loader = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=0,
                                                  drop_last=False)
    elif args.data == 'mnist':
        np.random.seed(args.seed)
        target_transform = np.random.permutation(num_classes)
        trainset = MNIST(root=testdir, train=False, download=False, transform=transform_test,
                         target_transform=None, index=index)
        test_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, drop_last=False)
    elif args.data == 'fashion_mnist':
        np.random.seed(args.seed)
        target_transform = np.random.permutation(num_classes)
        trainset = Fashion_MNIST(root='D:\Dataset\/fashion_mnist', train=False, download=False, transform=transform_test,
                                 target_transform=None, index=index)
        test_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, drop_last=False)
