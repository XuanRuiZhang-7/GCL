...
from MNIST import Mnist_Generator, Mnist_Discriminator, Mnist_Generator_ConvT, Mnist_Discriminator_ConvT
...
def save_samples(args, generator, current_task, epoch, pre_index, num_class_per_task):
    # target_labe=l = 1
    # 生成指定target_label的图片
    # ind = list(range(len(pre_index)+num_class_per_task))
    # p_i = ind = list(range(len(pre_index)+num_class_per_task))
    """
    ind = list(range(10))
    p_i = list(range(10))
    ...
    """
    if not os.path.exists('./results/' + args.data + '/' + args.G_type):
        os.makedirs('./results/' + args.data + '/' + args.G_type)
    # sample_num = len(pre_index)*10 + num_class_per_task*10
    sample_num = 100
    ...
    torchvision.utils.save_image(fake_data * 0.5 + 0.5,
                                 './results/%s/%s/task_%d_epoch_%d_grid.png' % (
...
def train_task(args, trainset, test_loader, current_task, prototype={}, pre_index=0):
    ...
    if 'imagenet' in args.data:
        ...
        if 'linear' in args.G_type:
            model = models.create('resnet18_mnist_32', pretrained=False, feat_dim=args.feat_dim, embed_dim=args.num_class)
        elif 'convt' in args.G_type:
            model = models.create('resnet18_mnist_128', pretrained=False, feat_dim=args.feat_dim, embed_dim=args.num_class)
            # model = models.create('resnet18_mnist_32', pretrained=False, feat_dim=args.feat_dim, embed_dim=args.num_class)
    ...
    if current_task == 0:
        if 'linear' in args.G_type:
            ...
            # 128
            generator = Mnist_Generator_ConvT(channel=channel, n_classes=args.num_class, nz=args.latent_dim, ngf=64)
            discriminator = Mnist_Discriminator_ConvT(channel=channel, n_classes=args.num_class, ndf=64)
            # 32
    ...
    # optimizer_G_ = torch.optim.RMSprop(lr=0.0008, clipvalue=1.0, decay=1e-8)
    ...
    if current_task > 0:
        model = model.eval()
    # Curriculum Learning
    data_sizes = get_cycle_sizes(start_percent=0.256, end_percent=1.0, multiplier=0.5, epochs=300 + 1)
    print(data_sizes)
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in tqdm(enumerate(train_loader, 0)):
            ...
            inputs, labels = inputs1, labels1  # !  inputs=[bs,1,32] label=[bs]
            # print(inputs.shape)

            # ## Classification loss
            embed_feat = model(inputs)
            if current_task == 0:
                # print(embed_feat.shape)   图片reshape到128-》bs*8192
                soft_feat = model.embed(embed_feat)
            ...
            if epoch == 0 and i == 0:
                print(50 * '#')
        scheduler.step()
        loss_log['per_epoch_time'] += (time.time() - epoch_start_time)
        ...
        with torch.no_grad():
            ...
            ave += acc
            num_class += len(tmp)
        print('[Metric Epoch %05d]\t Total Loss: %.3f \t LwF Loss: %.3f \tLwF Loss*old_task_factor: %.3f \t acc: %.3f \tper_epoch_time:%.3f'
              % (epoch + 1, loss_log['C/loss'], loss_log['C/loss_aug'], loss_log['C/loss_aug']*old_task_factor, acc, loss_log['per_epoch_time']))
    ...
    loss_log['total_time'] = time.time() - start_time
    loss_log['avg_epoch_time'] = np.mean(loss_log['per_epoch_time'])
    ...
    start_time = time.time()
    if current_task != args.num_task+1:
            ...
            for i, data in tqdm(enumerate(train_loader, 0)):    # tqdm 进度条工具
                ...
                if i % args.n_critic == 0:
                    ...
                    if current_task == 0:
                        ...
                        # pdb.set_trace()
                        pre_feat = Variable(pre_feat, requires_grad=False)
                        pre_feat_old = Variable(pre_feat_old, requires_grad=True)
                        loss_aug = loss_mse(pre_feat, pre_feat_old)     # 回放对齐（均方误差损失）
                        # loss_aug_ssim = 1-ssim_loss(pre_feat, pre_feat_old)
                        # loss_aug = loss_aug_mse + 0.5 * loss_aug_ssim
                    ...
                    if i % 10 == 0:
                        print('[GAN Epoch %05d]\t Dp Loss: %.5f Dn Loss: %.5f \t G Loss: %.5f \t aug Loss: %.3f\t LwF Loss: %.5f' % (
                            epoch + 1, d_loss_cgan_p.item(), d_loss_cgan_n.item(), g_loss.item(),loss_aug.item(), loss_log['G/prev_rf']))

                    # 每50个epoch保存图片
                    if epoch % 2 == 0 and i == 0:
                        save_samples(args,generator,current_task, epoch,pre_index,num_class_per_task)

            print('[GAN Epoch %05d]\t D Loss: %.3f \t G Loss: %.3f \t LwF Loss: %.3f\t  per_epoch_time: %.3f' % (
                epoch + 1, loss_log['D/loss'], loss_log['G/loss'], loss_log['G/prev_rf'],loss_log['per_epoch_time']))
        ...
        loss_log['total_time'] = time.time() - start_time
        loss_log['avg_epoch_time'] = np.mean(loss_log['per_epoch_time'])
        print("Avg one epoch time: %.2f, total %d epochs time: %.2f" % (np.mean(loss_log['per_epoch_time']),
                                                                        args.epochs_gan, loss_log['total_time']))
    tb_writer.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-G_type', default='linear', choices=['linear', 'convt', 'infoGAN'], help='type of generator')
    ...
    parser.add_argument('-nThreads', '-j', default=1, type=int, metavar='N', help='number of data loading threads') #
    ...
    parser.add_argument('-gan_lr_decay_step', type=float, default=80, help='Decay learning rate every x steps')

    parser.add_argument('-latent_dim', type=int, default=100, help="learning rate of new parameters")   #
    ...
    parser.add_argument('-epochs_gan', default=301, type=int, metavar='N', help='epochs for training process')  #
...
######
    ...
    if 'mnist' in args.data:    # 28*28 mnist\fashion_mnist
        if 'linear' in args.G_type:
            transform_train = transforms.Compose([
                transforms.ToTensor(),
    ...
    for i in range(args.start, num_task + 1):
        ...
        if args.data == 'cifar100':
            ...
            np.random.seed(args.seed)
            target_transform = np.random.permutation(num_classes)
            trainset = Fashion_MNIST(root=traindir, train=True, download=False, transform=transform_train,
                             target_transform=target_transform, index=class_index)
            train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.BatchSize, shuffle=True,
                                                       num_workers=args.nThreads, drop_last=True)
